...
int main( int argc, char* argv[] )
{
	...
	for( int argn = 1; argn < argc; ++argn )
	{
		...
		{
			language = Language_HLSL_10;
		}
		else if( String_Equal( arg, "-legacyhlsl" ) )
		{
			language = Language_HLSL_9;
		}
	...
	else if (language == Language_HLSL_10)
	{
		HLSLGenerator generator;
		if (!generator.Generate( &tree, HLSLGenerator::Target(target), entryName, language == Language_HLSL_9 ))
		{
			Log_Error( "Translation failed, aborting\n" );
			return 1;
		}
